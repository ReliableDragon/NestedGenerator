...
class WeightedChoice():
    ...
    def __str__(self):
        return f'({self.weight}) "{self.choice}"'
...
class NestedChoices():
    ...
    def choices_string_to_tree(choices_string):
        ...
        for top_level_choice_data in top_level_choices_list:
            ...
            for choice in nested_choices:
                ...
                # If indent is not increased, the last value was not a parent.
                if new_indent == indent:
                    parent_choicedict_stack.pop()
                    current_dict = parent_choicedict_stack[-1]
                # The indent has decreased, and we need to remove the previous
                # value, which was not a parent, as well as the parent before
                # that, as we've travelled back up a level.
                ...
                indent = new_indent
                choice = choice[indent:]

                try:
                    weighted_choice = WeightedChoice(*choice.split(' ', 1), tag=wc_tag)
                except TypeError:
                    # Empty choice.
                    weighted_choice = WeightedChoice(choice, '', wc_tag)
    ...
    # from it. Parameters can be passed to affect how it runs. 'num' says how many
    # choices to generate, and 'uniqueness_level' says at what level the returned
    # choices should be unique. -1 means the lowest level, simply don't repeat the
    # exact same choice, while positive values starting from 1 indicate which level
    # of indentation will be removed if a previous choice chose it.
    def gen_choice(self, params={'num': 1, 'uniqueness_level': -1}):
        generated_choices = []
        used_choices = []
    ...
    def make_replacements(self, generated_choice, weighted_choice):
        # print(weighted_choice)
        generated_choice = generated_choice.replace('$', weighted_choice.choice)
...
def pick_choice(filtered_choices):
    ...
    assert total_weight > 0, "Total weight was <= 0, most likely you wrote a generation that removed all valid choices from a config."
...
def recursive_dict_print(dict, indent):
    val = ''
    indent_str = ' ' * indent
...
def gen_choice_bak(choices, params={'num': 1, 'uniqueness_level': -1}):
    ...
    level = 0
    while not generated:
        weights_and_choices = [choice.split(' ', 1) for choice in choices]
        weighted_choices = list(map(lambda wandp: (int(wandp[0]), wandp[1]), weights_and_choices))

        total_weight = sum([i for i, _ in weighted_choices])
        rand = random.randint(1, total_weight)

        i = 0
        while rand > 0:
            rand -= weighted_choices[i][0]
            if rand > 0:
                i += 1
        choice = weighted_choices[i][1]

        if '$' not in choice:
            generated_choice = generated_choice.replace('$', choice)
            generated = True
        else:
            level += 2
            choices = re.split(f'\r?\n {{{level}}}(?! )' , choice)

            generated_choice = generated_choice.replace('$', choices[0])
...
if __name__ == "__main__":
    ...
    print(choices.gen_choice(params={'num': 5, 'uniqueness_level': -1}))
